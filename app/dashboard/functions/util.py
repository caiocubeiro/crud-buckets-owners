...
def montaPieChartBuckets(df):
    ...
    fig = go.Figure(
        data=[
            go.Pie(
                labels=grouped_df["department"],
                values=grouped_df["qtd_buckets"],
                hole=0.3,
            )
        ]
    )
    ...
    fig.update_layout(
        title="Quantidade de Buckets por Departamento", template="plotly_white"
    )
    return fig
...
def montaBarChartSize(df):
    ...
    fig = go.Figure(
        data=[go.Bar(name="Size", x=grouped_df["owner"], y=grouped_df["gbs"])]
    )
...
def montaPieChartSize(df):
    ...
    fig = go.Figure(
        data=[
            go.Pie(labels=grouped_df["department"], values=grouped_df["gbs"], hole=0.3)
        ]
    )

    fig.update_layout(
        title="Quantidade de Espaço Alocado por Departamento", template="plotly_white"
    )
    return fig
